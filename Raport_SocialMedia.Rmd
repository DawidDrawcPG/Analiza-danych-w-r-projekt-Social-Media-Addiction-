...
```{r podglad, results='asis', echo=FALSE, message=FALSE, warning=FALSE}
library(dplyr)
library(tibble)
library(knitr)

cat("Liczba wierszy:", nrow(d), "<br>")
cat("Liczba kolumn:", ncol(d), "<br><br>")
...
# Podgląd pierwszych 8 wierszy (jedna tabela + suwak)
d_head <- head(d, 8)

if (knitr::is_html_output()) {
  cat("<style>
       .scroll-x {overflow-x:auto; width:100%; border:1px solid #ddd; padding:8px; border-radius:8px;}
       .scroll-x table {width: max-content;}
       </style>")

  cat("<div class='scroll-x'>")
  cat(as.character(knitr::kable(
    d_head,
    format  = "html",
...
```{r statystyki, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
stats_tbl_long <- num_tbl %>%
  ...
  pivot_longer(
    ...
    names_pattern = "^(.*)_([^_]*)$",   # rozdział po OSTATNIM "_"
    values_to = "Wartosc"
  ) %>%
  pivot_wider(
    names_from = "Miara",
  ...
  mutate(
    across(c(srednia, mediana, sd, min, max), ~round(.x, 2)),
    n = as.integer(n)
  ) %>%
  arrange(Zmienna)
...
knitr::kable(
  stats_tbl_long,
  caption = "Statystyki opisowe dla zmiennych liczbowych"
)
...
### 4.1 Korelacje: czas w SM vs reszta {#korelacje_czas}

```{r korelacje_czas, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=6}
library(dplyr)
...
### 4.2 Wizualizacja korelacji (lollipop) {#korelacje_lollipop}
...
```{r korelacje_lollipop, echo=FALSE, message=FALSE, warning=FALSE, fig.width=9, fig.height=6}
...
### 4.3 TOP 10 korelacji {#korelacje_top10}

```{r korelacje_top10, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
library(dplyr)
...
```{r nieparam, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
...
# formatowanie do tabeli (żeby nie było 0e+00)
fmt_num <- function(x, d = 4) {
  ifelse(is.na(x), "", formatC(x, format = "f", digits = d))
}
fmt_p <- function(p) {
  ifelse(is.na(p), "", formatC(p, format = "f", digits = 4))
}
...
if ("Affects_Academic_Performance" %in% names(d) && length(num_yesno) > 0) {
  ...
  # formatowanie WYŚWIETLANIA (liczby -> tekst)
  tests_yesno_print <- tests_yesno %>%
    mutate(
      diff_mean_yes_minus_no = fmt_num(diff_mean_yes_minus_no, 4),
      t_p      = fmt_p(t_p),
      wilcox_p = fmt_p(wilcox_p),
      cohen_d  = fmt_num(cohen_d, 4)
    )

  knitr::kable(
    tests_yesno_print,
    format = if (knitr::is_html_output()) "html" else "pipe",
    escape = FALSE,  # ważne przez "<0.001"
