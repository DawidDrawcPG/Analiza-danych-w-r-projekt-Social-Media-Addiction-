...
```{r korelacje, results='asis'}
num_vars <- d %>%
  select(where(is.numeric)) %>%
  select(-any_of("Student_ID"))

# Spearman: lepszy dla skal punktowych (0-10) i danych z remisami
cor_mat <- cor(num_vars, use = "pairwise.complete.obs", method = "spearman")

# macierz p-value dla korelacji (żeby nie brać na wiarę samych kolorów)
...
# krótka tabelka: czas w SM vs kluczowe wskaźniki
if ("Avg_Daily_Usage_Hours" %in% names(d)) {
  key_vars <- c("Addicted_Score","Sleep_Hours_Per_Night","Mental_Health_Score","Conflicts_Over_Social_Media")
  key_vars <- intersect(key_vars, names(d))

  corr_key <- purrr::map_dfr(key_vars, function(v) {
  ...
  knitr::kable(
    corr_key,
    caption = "Wybrane korelacje Spearmana: czas w SM vs kluczowe wskaźniki",
    align = "l"
  )
}
```
